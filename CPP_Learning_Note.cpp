...
/*声明的三种方法
using namespace X; //引入整个名字空间，后续就不需要再加限定词了
using X::name ; //使用单个名字，后续yongname不用限定词
X::name; //程序中加上名字空间前缀，如X::
*/


2. C++的新的输入输出流库(头文件iostream)将输入输出看成一个流，并用
输出运算符 << 
输入运算符 >> 对数据(变量和常量进行输入输出);
...
以下为 a 和 b 交换失败的情况
原因：
1. 按值传递：在 swap 函数中，x 和 y 是 a 和 b 的副本。
   x 和 y 的修改不会影响 a 和 b 的值。
2. 局部变量：x 和 y 是 swap 函数的局部变量，
   它们的生命周期仅限于 swap 函数的执行期间。
   当 swap 函数执行完毕后，x 和 y 就会被销毁。
*/
...
8.对于不包含循环的简单函数，建议用inline关键字声明 为"inline内联函数"，
...
11.函数重载：C++允许函数同名，只要它们的形参不一样(个数或对应参数类型)，
...
14.1动态内存分配： 关键字 new 和 delete 比C语言的malloc/alloc/realloc和free更好，
...
栈（Stack）：用于存储局部变量、函数参数等。栈内存由编译器自动分配和释放，速度快但空间有限。

堆（Heap）：用于动态内存分配，程序员手动管理（通过 new 和 delete）。堆空间较大，但分配和释放速度较慢。

全局/静态存储区：用于存储全局变量和静态变量，程序结束时释放。
...
19.类体外定义方法（成员函数），必须在类定义中声明，类体外要有类作用域，否则就是全局
外部函数了！
#include <iostream>
using namespace std;
...
20.类模板：我们可以将一个类变成“类模板”或“模板类”，正如一个模板函数一样。
...
23.Inheritance继承(Derivation派生)： 一个派生类(derived class)
...
{
public:
    MM(string name,int age) :name(name),age(age){}  //初始化参数列表
    void print()
    {
...
初始化参数列表:在C++中，初始化参数列表（也称为成员初始化列表）是构造函数的一种语法，用于在对象创建时直接初始化类的成员变量，而不是在构造函数体内通过赋值语句来初始化。

ClassName::ClassName(参数列表) : 成员变量1(初始值1), 成员变量2(初始值2), ... {
    // 构造函数体
}
直接初始化成员变量：初始化参数列表会在构造函数体执行之前，直接初始化成员变量。
必须使用初始化列表的情况：初始化常量成员（const）;初始化引用成员（&）;初始化没有默认构造函数的类对象成员。
